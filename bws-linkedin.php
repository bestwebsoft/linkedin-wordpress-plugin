...
if ( ! function_exists( 'lnkdn_add_admin_menu' ) ) {
	function lnkdn_add_admin_menu() {
		bws_general_menu();
		$settings = add_submenu_page( 'bws_plugins', __( 'LinkedIn Settings', 'bws-linkedin' ), 'LinkedIn', 'manage_options', 'linkedin.php', 'lnkdn_settings_page' );
...
if ( ! function_exists( 'lnkdn_init' ) ) {
	function lnkdn_init() {
		...
		bws_wp_min_version_check( plugin_basename( __FILE__ ), $lnkdn_plugin_info, '3.8' );
...
if ( ! function_exists( 'lnkdn_settings_page' ) ) {
	function lnkdn_settings_page() {
		...
		<div class="wrap">
			...
			bws_plugin_reviews_block( $lnkdn_plugin_info['Name'], 'bws-linkedin' ); ?>
		</div>
	<?php }
}
...
if ( ! class_exists( 'Lnkdn_Main_Widget' ) ) {
	class Lnkdn_Main_Widget extends WP_Widget {
		...
		function widget( $args, $instance ) {
			$title 				= ( ! empty( $instance['lnkdn_title'] ) ) ? $instance['lnkdn_title'] : '';
...
if ( ! function_exists ( 'lnkdn_plugin_banner' ) ) {
	function lnkdn_plugin_banner() {
		...
		if ( isset( $_REQUEST['page'] ) && 'linkedin.php' == $_REQUEST['page'] ) {
